...
def main():
    ...
    while True:
        ...
        for i, data in enumerate(acc_data):
            gravity[i] = Gra_K * gravity[i] + (1 - Gra_K) * data
            lowpass_acc[i] = Law_K * lowpass_acc[i] + (1 - Law_K) * data
            filtered_acc[i] = lowpass_acc[i] - gravity[i]

        """
        合成加速度
        ・軸成分を排除した加速度データ。手振れなどの微小な揺れによるノイズを除去する
        """
        m = filtered_acc[0] ** 2 + filtered_acc[1] ** 2 + filtered_acc[2] ** 2
        ...
        """移動距離算出"""
        for i, acc in enumerate(filtered_acc):
            acc *= 9.81
            v[i] += acc * dt
        ...
        for i in range(3):
            ...
            plot_highacc[i].append(filtered_acc[i])
            plot_v[i].append(v[i])
            plot_x[i].append(x[i])
        print("\r{} / {} : {},{}".format(keikaTime, stop_time, Gra_K, Law_K), end="")
